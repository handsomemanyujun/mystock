...
public class TdxClient {
	...
	private Map<String,TdxLibrary>  tdxLibrarys;
	...
	public FundPoolDO queryFundDO(String userId) {
		...
		getTdxLibrary(userId).QueryData(clientID, 0, result, errInfo);
	...
	public List<StockDO> queryStockDO(String userId,String zqdm) {
		...
		getTdxLibrary(userId).QueryData(clientID, 1, result, errInfo);
		String res = Native.toString(result, "GBK");
		cleanResult();
		if(res !=null) {
			String[][] items =TdxResultUtil.parseStr(res);
	...
	public List<OrderDO> queryDelegate(String userId) {
		...
		getTdxLibrary(userId).QueryData(clientID, 2, result, errInfo);
		String res = Native.toString(result, "GBK");
		cleanResult();
		if(res !=null) {
			String[][] items =TdxResultUtil.parseStr(res);
	...
	public OnlinePriceDO queryMarket(String userId,String zqdm) {
		getTdxLibrary(userId).GetQuote(clientID, zqdm, result, errInfo);
		String res = Native.toString(result, "GBK");
		cleanResult();
	...
	public void cancleOrder(String userId,OrderDO orderDO) {
		...
		if(TdxResultUtil.isSHCode(orderDO.getZqCode())) {	// 上证
			getTdxLibrary(userId).CancelOrder(clientID, "1", orderDO.getOrderId(), result, errInfo);
		} else {
			getTdxLibrary(userId).CancelOrder(clientID, "0",  orderDO.getOrderId(), result, errInfo);
		}
		cleanResult();
	}
	...
	public void crateOrder(String userId, OrderDO orderDO) {
		...
		log.info("下单，" + orderDO);
		getTdxLibrary(userId).SendOrder(clientID, orderDO.isBuy() ? 0
	...
	@PostConstruct
	public void init() {
		tdxLibrarys.put("62124349", getLibrary("62124349","122541","A238440910,0129170967"));
		tdxLibrarys.put("62278700", getLibrary("62278700","324877","A317676125,0187394552"));
	}
	
	private TdxLibrary getLibrary(String userId,String passWord,String gddm) {
		...
		TdxLibrary tdxLibrary = (TdxLibrary) Native.loadLibrary("trade", TdxLibrary.class);
		tdxLibrary.OpenTdx();
	...
	private TdxLibrary getTdxLibrary(String userId) {
		if(DateUtil.isOpenTradeTime()){
			throw new RuntimeException("市场未到开市时间，无法操作");
		}
		return tdxLibrarys.get(userId);
	}
	
	public void close() {
		/*getTdxLibrary(userId).Logoff(clientID);
		getTdxLibrary(userId).CloseTdx();*/
	}
